...
public class Hero extends Character {
	...
	public void push(Vector2 direction) {
        ...
        // update velocity
        position.velocity = (position.velocity.add(direction.add(GameWorld.Gravity))).mult(drag);

        if (position.velocity.getX() < 0)
        {
            position.velocity.setX(Math.max(position.velocity.getX(), -1 * horizontalSpeedLimit));
        }
        else
        {
            position.velocity.setX(Math.min(position.velocity.getX(), horizontalSpeedLimit));
        }
	}
